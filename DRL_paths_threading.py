...
import sys
# sys.path.insert(0,'SDNapps_proac/RoutingGeant/DRL/dRSIR/23nodos')
# sys.path.insert(0,'SDNapps_proac/RoutingGeant/DRL/dRSIR/32nodos')
import setting
"""Modified 2/1/2023 Issue with running file"""
sys.path.insert(0,'DRL/32nodes')
# sys.path.insert(0,'SDNapps_proac/RoutingGeant/DRL/dRSIR/64nodos')
# sys.path.insert(0,'SDNapps_proac/RoutingGeant/DRL/dRSIR/48nodos')
import time
# import bot
import json,ast
import csv
...
thread_count = setting.drl_thread
# def call_bot(msg):
        # bot.sendMessage(msg)
...
def get_paths_base():
    file_base = 'DRL/'+str(num_nodes)+'nodes/paths_weight.json'
...
def get_paths_DRL():
    file_DRL = 'DRL/'+str(num_nodes)+'nodes/drl_paths.json'
...
def calc_all_stretch(cont):
    ...
    a = time.time()
    with open('DRL/'+str(num_nodes)+'nodes/stretch/'+str(cont)+'_stretch.csv','w') as csvfile:
...
def get_all_paths(episode_rewards, episode_states_all, episode_duration_all, episodes):
    ...
    file = 'DRL/'+str(num_nodes)+'nodes/k_paths.json'
    ...
    #write choosen paths
    with open('DRL/'+str(num_nodes)+'nodes/drl_paths.json','w') as json_file:
...
def DRL_thread(): #cambiar para que lo llame a drl
    ...
    while count < thread_count.value:  
        ...
        time_stretch = calc_all_stretch(count)
        iteration_times.append(time_DRL) # print('time_stretch' , time_stretch)
        sleep = setting.MONITOR_PERIOD - time_DRL - time_stretch # We need to review what is time_DRL and Time_Stretch
        
        if sleep > 0:
            print("**"+str(count)+"**time remaining drl and stretch",sleep)
            time.sleep(sleep)
        else:
            print("**"+str(count)+"**time remaining drl and stretch",sleep)        
            time.sleep(0.2)
    ...
    file_info_eps = 'DRL/'+str(num_nodes)+'nodes/episodes_info.txt'
    list_of_lines = ["Episodes: "+str(episodes),"Iterations: "+str(count),"Time iteration: "+str(iteration_times)+"\n",str(episode_rewards)+"\n",str(episode_states_all)+"\n", str(episode_duration_all)+"\n"]
