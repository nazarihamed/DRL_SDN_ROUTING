...
thread_count = Config.drl_thread
# def call_bot(msg):
        # bot.sendMessage(msg)
...
def get_paths_base():
    file_base = 'SDNapps_proac/RoutingGeant/DRL/dRSIR/'+str(num_nodes)+'nodos/paths_weight.json'
...
def get_paths_DRL():
    file_DRL = 'SDNapps_proac/drl_paths.json'
...
def calc_all_stretch(cont):
    ...
    a = time.time()
    with open('SDNapps_proac/stretch/'+str(cont)+'_stretch.csv','w') as csvfile:
...
def get_all_paths(episode_rewards, episode_states_all, episode_duration_all, episodes):
    ...
    file = 'SDNapps_proac/RoutingGeant/DRL/dRSIR/'+str(len(env.topo_nodes))+'nodos/k_paths.json'
    ...
    #write choosen paths
    with open('SDNapps_proac/drl_paths.json','w') as json_file:
...
def DRL_thread(): #cambiar para que lo llame a drl
    ...
    file_info_eps = "SDNapps_proac/episode_info.txt"
